package ticketing.order.handler;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> onApi(ApiException ex, HttpServletRequest req) {
        ...
        HttpStatus status =
                ...
                Errors.VALIDATION_FAILED.equals(ex.code()) ? HttpStatus.UNPROCESSABLE_ENTITY : HttpStatus.BAD_REQUEST;

        return ResponseEntity.status(status).body(new ErrorResponse(ex.code(), ex.getMessage(), traceId));
    }
    ...
    public ResponseEntity<ErrorResponse> onEtc(Exception ex, HttpServletRequest req) {
        String traceId = (String) req.getAttribute(TraceIdFilter.HEADER);
        return ResponseEntity.internalServerError().body(new ErrorResponse("INTERNAL_ERROR", ex.getMessage(), traceId));
    }
}
